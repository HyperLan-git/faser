...
void BiquadFilter::updateParameters() {
    ...
    const float A = (float)std::pow(10, this->parameters.gain / 40),
                sqrtA = std::sqrt(A);
    const float om =
        3.141592653589793f * 2 * this->parameters.f / this->sampleRate;
    const float a = std::sin(om) / (2 * this->parameters.Q);
    const float cs = std::cos(om);
    switch (this->type) {
        case ALLPASS:
            this->coefficients = {1 - a, -2 * cs, 1 + a, 1 + a, -2 * cs, 1 - a};
            break;
        case LOWPASS:
            this->coefficients = {(1 - cs) / 2, 1 - cs,  (1 - cs) / 2,
                                  1 + a,        -2 * cs, 1 - a};
            break;
        case HIGHPASS:
            this->coefficients = {(1 + cs) / 2, -1 - cs, (1 + cs) / 2,
            ...
            break;
        case BANDPASS:
            this->coefficients = {a, 0, -a, 1 + a, -2 * cs, 1 - a};
            break;
        case NOTCH:
            this->coefficients = {1, -2 * cs, 1, 1 + a, -2 * cs, 1 - a};
            break;
        case PEAK:
            this->coefficients = {1 + a * A, -2 * cs, 1 - a * A,
                                  1 + a / A, -2 * cs, 1 - a / A};
            break;
        case LOWSHELF:
            this->coefficients = {A * ((A + 1) - (A - 1) * cs + 2 * sqrtA * a),
                                  2 * A * ((A - 1) - (A + 1) * cs),
