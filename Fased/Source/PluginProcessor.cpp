...
void FasedAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer,
    ...
    float* r = inputs < 2 || outputs < 2 ? nullptr : buffer.getWritePointer(1);

    const int n = *filters;
    const float fc = *freq, q = *Q, g = (*gain) / n;
    enum BiquadFilterType t =
        static_cast<enum BiquadFilterType>(type->getIndex() + 1);

    if (t != filter.getType()) {
        std::memset(this->states_l, 0, sizeof(this->states_l));
        std::memset(this->states_r, 0, sizeof(this->states_r));
    }
    ...
    if (t != filter.getType() || b.f != fc || b.Q != q || b.gain != g ||
        ...
        if (prevFilters != n && n < MAX_FILTERS) {
            std::memset(this->states_l + n * 2, 0,
                        sizeof(this->states_l) - 2 * n * sizeof(float));
            std::memset(this->states_r + n * 2, 0,
                        sizeof(this->states_r) - 2 * n * sizeof(float));
        }
    ...
    filter.processBlockMul(l, samples, this->states_l, n);
    if (r) filter.processBlockMul(r, samples, this->states_r, n);
    this->prevFilters = n;
}
